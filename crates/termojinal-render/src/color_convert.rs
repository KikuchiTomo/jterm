//! Convert `termojinal_vt::Color` to GPU-friendly `[f32; 4]` RGBA values.
//!
...
impl ThemePalette {
    ...
    /// takes individual string slices to avoid coupling `termojinal-render` to the
...
/// Convert a `termojinal_vt::Color` to an RGBA `[f32; 4]` using the given palette.
///
...
/// Convert a `termojinal_vt::Color` to an RGBA `[f32; 4]` suitable for the GPU.
///
...
mod tests {
    ...
    #[test]
    fn test_named_red() {
        let c = color_to_rgba(Color::Named(termojinal_vt::NamedColor::Red), true);
    ...
    fn test_named_bright_white() {
        let c = color_to_rgba(Color::Named(termojinal_vt::NamedColor::BrightWhite), true);
    ...
    fn test_themed_named_uses_palette() {
        ...
        let c = color_to_rgba_themed(Color::Named(termojinal_vt::NamedColor::Red), true, &palette);
        assert_eq!(c, [1.0, 0.0, 0.0, 1.0]);
    }
