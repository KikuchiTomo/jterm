...
impl CommandRunner {
    ...
    /// - `stderr` inherited (so diagnostic output goes to termojinal's log)
    /// - Working directory set to the command's directory
    /// - `TERMOJINAL_SOCKET` env var pointing to termojinal's IPC socket
    ...
    ///
    /// This is useful for testing where no real termojinal socket exists.
...
/// Get the default termojinal IPC socket path.
fn default_socket_path() -> String {
    termojinal_session::daemon::socket_path()
}
